...
import { saveCookies, loadCookies, clearCookies, hasStoredCookies, getAuthState, getCookiesPath, } from "./auth.js";
...
export class ChipotleSession {
    ...
    async status() {
        ...
        if (!hasCookies) {
            return {
                ...
                hasCookies: false,
                cookiesPath,
                message: "Not logged in. Use chipotle_login to authenticate with your Chipotle account.",
            };
        }
    ...
    async login(email, password) {
        ...
        try {
            ...
            // Click Sign In
            const signInBtn = p.locator('button:has-text("Sign In"), a:has-text("Sign In"), [data-testid="sign-in"]');
            if (await signInBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
                await signInBtn.first().click();
                await p.waitForTimeout(2000);
            }
            // Fill email
            const emailInput = p.locator('input[type="email"], input[name="email"], input[placeholder*="email" i]');
            ...
            // Submit
            const submitBtn = p.locator('button[type="submit"], button:has-text("Log In"), button:has-text("Sign In")');
    ...
    async logout() {
        ...
        return {
            success: true,
            message: "Logged out. Session cookies cleared from ~/.striderlabs/chipotle/cookies.json",
        };
    }
    // -----------------------------------------------------------------------
    // search_locations
    // -----------------------------------------------------------------------
    async searchLocations(address, radius) {
        const p = await this.getPage();
        const ctx = await this.getContext();
        try {
            // Use Chipotle's location search API endpoint
            const encoded = encodeURIComponent(address);
            await p.goto(`${ORDER_URL}/en-us/locations?query=${encoded}`, { waitUntil: "domcontentloaded", timeout: DEFAULT_TIMEOUT });
            await p.waitForTimeout(3000);
            const locations = [];
            // Try to scrape location cards
            const locationCards = p.locator('[data-testid="restaurant-card"], [class*="LocationCard"], [class*="location-card"], article[class*="location"]');
            ...
            for (let i = 0; i < Math.min(count, 10); i++) {
                ...
                try {
                    ...
                    const address = (await card
                        ...
                        .textContent()
                        .catch(() => "")) || "";
                    const hours = (await card
                        .locator("[class*='hours'], [class*='time']")
                        .first()
                        .textContent()
                        .catch(() => "")) || "";
                    const distance = (await card
            ...
            if (locations.length === 0) {
                const apiResponse = await p.evaluate(async (addr) => {
                    try {
                        const resp = await fetch(`https://services.chipotle.com/restaurant/v3/restaurant?restaurantStatuses=OPEN,LIMITED&orderBy=PROXIMITY&address=${encodeURIComponent(addr)}&pageSize=10`, {
                            headers: {
                ...
                if (apiResponse &&
                    Array.isArray(apiResponse.data?.restaurants)) {
                    const restaurants = apiResponse.data.restaurants;
    ...
    async getLocation(locationId) {
        ...
        const ctx = await this.getContext();
        try {
            const apiResponse = await p.evaluate(async (id) => {
                try {
                    const resp = await fetch(`https://services.chipotle.com/restaurant/v3/restaurant/${id}`, {
                        headers: {
                            Accept: "application/json",
                            "Chipotle-CorrelationId": crypto.randomUUID(),
                        },
            ...
            if (apiResponse && apiResponse.restaurantNumber) {
                const r = apiResponse;
                await saveCookies(ctx);
                return {
                    success: true,
            ...
            const categories = [
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            id: "bowl",
                            name: "Burrito Bowl",
                            description: "Your choice of freshly grilled meat or sofritas served in a bowl with rice, beans, or fajita veggies",
                            price: 9.80,
                        ...
                        },
                        {
                            id: "tacos",
                            name: "Tacos",
                            description: "Three tacos with your choice of freshly grilled meat or sofritas and toppings",
                            price: 9.80,
                        ...
                        },
                        {
                            id: "salad",
                            name: "Salad",
                            description: "Romaine lettuce base with your choice of protein and fresh toppings",
                            price: 9.80,
                        ...
                        },
                        {
                            id: "quesadilla",
                            name: "Quesadilla",
                            description: "Flour tortilla filled with Monterey Jack cheese and your choice of protein",
                            price: 11.30,
    ...
    // start_order
    // -----------------------------------------------------------------------
    async startOrder(entreeType, locationId) {
    ...
    async customizeItem(customizations) {
        if (!this.orderBuilder.entreeType) {
            return {
                success: false,
                message: "No order in progress. Call chipotle_start_order first to begin building an entree.",
            };
        }
        ...
        if (customizations.protein) {
            ...
            if (validProteins.some((p) => protein.includes(p))) {
                ...
                warnings.push(`Unknown protein "${customizations.protein}". Valid: ${validProteins.join(", ")}`);
            }
        }
        if (customizations.rice !== undefined) {
            ...
            if (validRice.some((r) => rice.includes(r) || r.includes(rice))) {
                ...
                warnings.push(`Unknown rice "${customizations.rice}". Valid: ${validRice.join(", ")}`);
            }
        }
        if (customizations.beans !== undefined) {
            ...
            if (validBeans.some((b) => beans.includes(b))) {
                ...
                warnings.push(`Unknown beans "${customizations.beans}". Valid: ${validBeans.join(", ")}`);
            }
        }
    ...
    // add_to_bag
    // -----------------------------------------------------------------------
    async addToBag(quantity = 1) {
        if (!this.orderBuilder.entreeType) {
            return {
                success: false,
                message: "No item to add. Call chipotle_start_order and chipotle_customize_item first.",
            };
        }
        const p = await this.getPage();
        const ctx = await this.getContext();
        ...
        if (this.orderBuilder.protein)
            customizations.push(this.orderBuilder.protein);
        ...
        try {
            ...
            const addBtn = p.locator('button:has-text("Add to Bag"), button:has-text("Add to Order"), [data-testid="add-to-bag"]');
        ...
        try {
            ...
            if (options.fulfillment === "delivery") {
                ...
                if (options.deliveryAddress) {
                    const addrInput = p.locator('input[placeholder*="address" i], input[name*="address" i]');
            ...
            for (let i = 0; i < count; i++) {
                ...
                const name = (await item.locator("span, p").first().textContent().catch(() => "")) || "";
            ...
            // Place order
            const placeOrderBtn = p.locator('button:has-text("Place Order"), button:has-text("Submit Order"), [data-testid="place-order"]');
        ...
        try {
            ...
            if (!orderId) {
                const recentOrder = p
                    ...
                    .first();
                if (await recentOrder.isVisible({ timeout: 3000 }).catch(() => false)) {
