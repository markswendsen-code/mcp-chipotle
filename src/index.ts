...
const server = new Server(
  { name: "strider-chipotle", version: "0.1.0" },
  { capabilities: { tools: {} } }
);

// Singleton session
let session: ChipotleSession | null = null;

async function getSession(): Promise<ChipotleSession> {
  if (!session) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      ...
      case "chipotle_login":
        return wrap(await s.login(a.email as string, a.password as string));
      ...
      case "chipotle_search_locations":
        return wrap(
          await s.searchLocations(a.address as string, a.radius as number | undefined)
        );

      case "chipotle_get_location":
        return wrap(await s.getLocation(a.locationId as string));

      case "chipotle_get_menu":
        return wrap(await s.getMenu(a.locationId as string | undefined));
      ...
      case "chipotle_start_order":
        return wrap(
          await s.startOrder(
            a.entreeType as string,
            a.locationId as string | undefined
          )
        );

      case "chipotle_customize_item":
        return wrap(
          await s.customizeItem({
            protein: a.protein as string | undefined,
            rice: a.rice as string | undefined,
            beans: a.beans as string | undefined,
            toppings: a.toppings as string[] | undefined,
            extras: a.extras as string[] | undefined,
          })
        );

      case "chipotle_add_to_bag":
        return wrap(await s.addToBag((a.quantity as number) || 1));
      ...
      case "chipotle_checkout":
        return wrap(
          await s.checkout({
            fulfillment: a.fulfillment as "pickup" | "delivery",
            pickupTime: a.pickupTime as string | undefined,
            deliveryAddress: a.deliveryAddress as string | undefined,
            confirm: (a.confirm as boolean) || false,
          })
        );

      case "chipotle_track_order":
        return wrap(await s.trackOrder(a.orderId as string | undefined));
